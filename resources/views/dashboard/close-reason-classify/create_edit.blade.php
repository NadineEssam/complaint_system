...
<div class="classify-form" dir="rtl">
  ...
  <div class="main-card">
    ...
    <div class="form-header">
      ...
      <div>
        <h4>{{ isset($classification) ? 'تعديل التصنيف' : 'إضافة تصنيف جديد' }}</h4>
        <p>يمكنك {{ isset($classification) ? 'تحديث' : 'إضافة' }} بيانات التصنيف بسهولة.</p>
      </div>
    </div>
          ...
          <div class="row">
            ...
            <div class="col-md-6 mb-4">
              <label class="form-label">
                السبب الرئيسي <span class="required-star">*</span>
              </label>
              <select class="form-select @error('fk_close_reason_id') is-invalid @enderror"
                name="fk_close_reason_id" required>
            ...
            <div class="col-md-6 mb-4">
              ...
              <select class="form-select @error('validity') is-invalid @enderror" name="validity">
                <option value="1" {{ old('validity', $classification->validity ?? 1) == 1 ? 'selected' : '' }}>فعّال</option>
                <option value="0" {{ old('validity', $classification->validity ?? 1) == 0 ? 'selected' : '' }}>غير فعّال</option>
              </select>
              @error('validity')
              <div class="error-text">{{ $message }}</div>
              @enderror
