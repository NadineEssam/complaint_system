...
<div class="classify-show" dir="rtl">
    ...
    <nav aria-label="breadcrumb">
      <ol class="breadcrumb">
        <li class="breadcrumb-item active">
          عرض التصنيف #{{ $classification->close_reason_classify_id }}
        </li>
        <li class="breadcrumb-item">
          <a href="{{ route('close-reason-classify.index') }}">التصنيفات</a>
        </li>
        <li class="breadcrumb-item">
  ...
  <div class="main-card">
    ...
    <div class="form-header">
      ...
      <div>
        <h4>تفاصيل التصنيف #{{ $classification->close_reason_classify_id }}</h4>
        <p>بيانات شاملة للتصنيف المحدد</p>
      </div>
    </div>
    ...
    <div class="card-body p-lg-4 p-3">
      ...
      <div class="action-buttons">
        ...
        <a href="{{ route('close-reason-classify.edit', $classification->close_reason_classify_id) }}"
          class="btn btn-warning btn-custom">
          <i class="bx bx-edit-alt"></i> تعديل
        </a>
        @endif

        <a href="{{ route('close-reason-classify.index') }}" class="btn btn-secondary btn-custom">
      ...
      <div class="detail-card">
        ...
        <div class="detail-row">
          ...
          <div class="detail-item">
            ...
            <span class="detail-value">{{ $classification->close_reason_classify_Name ?? '-' }}</span>
          </div>

          <div class="detail-item">
            <span class="detail-label">السبب الرئيسي</span>
            <span class="detail-value">{{ $classification->closeReason->close_reason_Name ?? '-' }}</span>
          </div>

          <div class="detail-item">
            <span class="detail-label">الحالة</span>
            <span class="status-badge {{ $classification->validity == 1 ? 'active' : 'inactive' }}">
              {{ $classification->validity == 1 ? 'فعّال' : 'غير فعّال' }}
            </span>
          </div>
      ...
      <div class="detail-card">
        ...
        <div class="detail-row">
          ...
          <div class="detail-item">
            ...
            <span class="detail-value">{{ $classification->createdBy->userID ?? '-' }}</span>
          </div>

          <div class="detail-item">
            <span class="detail-label">تاريخ الإنشاء</span>
            <span class="detail-value">
              {{ $classification->created_at ? $classification->created_at->format('d/m/Y H:i') : '-' }}
            </span>
          </div>

          <div class="detail-item">
            <span class="detail-label">آخر تعديل بواسطة</span>
            <span class="detail-value">{{ $classification->updatedBy->userID ?? '-' }}</span>
          </div>

          <div class="detail-item">
            <span class="detail-label">تاريخ آخر تعديل</span>
            <span class="detail-value">
              {{ $classification->updated_at ? $classification->updated_at->format('d/m/Y H:i') : '-' }}
            </span>
          </div>
