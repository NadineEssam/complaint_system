...
return new class extends Migration
{
    public function up(): void
    {
        Schema::table('comsources', function (Blueprint $table) {
            if (!Schema::hasColumn('comsources', 'created_by')) {
                $table->unsignedBigInteger('created_by')->nullable()->after('comsourcesname');
            }
            if (!Schema::hasColumn('comsources', 'updated_by')) {
                $table->unsignedBigInteger('updated_by')->nullable()->after('created_by');
            }
            if (!Schema::hasColumn('comsources', 'created_at')) {
                $table->timestamp('created_at')->nullable()->after('updated_by');
            }
            if (!Schema::hasColumn('comsources', 'updated_at')) {
